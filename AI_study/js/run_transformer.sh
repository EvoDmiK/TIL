...
int_port=$(cat ${CONFIG_PATH} | jq '.dovenest_api_int_port')
check=`ps -ef | grep ${int_port} | wc | awk '{print $1}'`

if [ $check -gt 1 ]; then
    echo "현재 transformer.js 가 실행 중 입니다."

else
    
    LOG_PATH=${ROOT_PATH}/logs/transformer

    echo "실행 코드가 있는 곳으로 이동 \n"
    cd ${ROOT_PATH}/TIL/AI_study/js
    echo 현재 작업 디렉토리 입니다.
    pwd
    ...
    LOG_PATH=${LOG_PATH}/${today}_dovenest_${idx}.log
    echo 이번에 생성할 로그 파일 경로 입니다. ${LOG_PATH}
    echo "transformer.js를 실행합니다."
