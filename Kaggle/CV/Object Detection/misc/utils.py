## CODE REFERENCE : https://github.com/pytorch/vision/blob/main/references/detection
import math as mt
import sys
import datetime
import errno
...
def reduce_dict(input_dict, avg = True):
    ...
    with torch.inference_mode():
        ...
        for k in sorted(input_dict.keys()):
            ...
            names.append(k)
            values.append(intput_dict[k])

        values = torch.stack(values, dim = 0)
        dist.all_reduce(values)

        if avg: values /= world_size
...
def train_one_epoch(model, optimizer, loader, device, epoch, print_freq, scaler = None):
    ...
    for image, target in loader:
        ...
        losses_reduced    = sum(loss for loss in loss_dict_reduces.values())
        loss_value        = losses_reduced.item()

        if not mt.isfinite(loss_value):
